...
>C#에서 메서드에 인자를 넘길 때는 크게 두 가지로 설명할 수 있다.
>**Call By Value (pass-by-value)** → **값을 복사해서 전달**하는 기본 방식
...
- `ref / out / in`은 **변수 자체를 참조로 전달**하여, 메서드가 **호출자 변수에 직접 접근**하게 만든다.
...
```
#### `out`
- **변수 자체를 참조**하지만, 목적이 출력이라서 메서드 내부에서 **반드시 값을 할당해야 한다.**
