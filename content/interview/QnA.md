...
8. ⭐️상속(Inheritance), 합성(Composition)
...
15. ⭐️`using` / `IDisposable` 개념(자원 해제)
...
18. `IEnumerable`, `IEnumerable<T>` 차이
...
19. ⭐️ `List<T>`, `Dictionary<TKey,TValue>`
...
20. `Dictionary` 키 비교 `Equals`, `GetHashCode` 차이
    - `Dictionary`는 키를 찾을 때 `GetHashCode`로 위치를 찾고 후보가 여러 개면 `Equals`같은 키인지 확인합니다.

21. ⭐️ 접근 제한자(`public`/`private`/`protected`/`internal`)
