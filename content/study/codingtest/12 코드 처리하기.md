...
```
- `code[i] == '1'`일 때만 `mode`를 토글하면 되는데 `mode`를 최상위 조건으로 나눠서
  (`switch/case`) 로직을 분리하니 오히려 복잡해졌다.
- 나머지는 하는 일이 거의 같아서 중복 코드가 많다. 
- 앞으로는 지문을 그대로 따라 쓰기보다 핵심 규칙을 먼저 생각하고 구성하는 습관을 들여야겠다.